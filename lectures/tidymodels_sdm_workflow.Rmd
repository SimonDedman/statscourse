...
best_params <- select_best(
  tune_results,
  metric = selection_metric  # set by prevalence check
)
...
`selection_metric` auto-set: **MCC** if prevalence >= 2.5%, **SEDI** if < 2.5% (Chicco & Jurman 2020; Wunderlich et al. 2019; PR [#630](https://github.com/tidymodels/yardstick/pull/630))

**Multiclass** (3+ classes, e.g. habitat type): both MCC and SEDI generalise — checked on min per-class prevalence
]
...
best_params <- select_best(tune_results, metric = selection_metric)
